...
public class ResponderSolicitudServicio extends IntentService{
	...
	protected void onHandleIntent(Intent intent) {
		...
		Intent intentBack = new Intent(Constantes.RESPONDER_SOLICITUD_AMISTAD_FILTRO_ACTION);
		...
		try{
			...
			if(accion.equalsIgnoreCase(Constantes.ACEPTAR_SOLICITUD)){
				...
				respuesta = restTemp.exchange(Constantes.ACEPTAR_SOLICITUD_AMISTAD_SERVICE_URL, 
						HttpMethod.POST, requestEntity, String.class);	
			}else{
				
				respuesta = restTemp.exchange(Constantes.RECHAZAR_SOLICITUD_AMISTAD_SERVICE_URL, 
						HttpMethod.POST, requestEntity, String.class);	
			}
			
			if (respuesta.getStatusCode() == HttpStatus.OK) {
				intentBack.putExtra("respuesta", Constantes.MSG_SOLICITUD_RESPONDIDA_OK);
			} else {
